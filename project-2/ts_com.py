...
def fillTable():
    ...
    try:
        ...
        print("[TS_COM]: File {} not found".format(file_name))
    finally:
        f.close()
...
def server():
    try:
        ...
        print("[TS_COM]: TS Socket sucessfully created")
    ...
    print("[TS_COM]: Host name : {}".format(host))
    host_ip = (socket.gethostbyname(host))
    print("[TS_COM]: IP address : {}".format(host_ip))
    print("[TS_COM]: Waiting on Connection Request...")
    ...
    while True:
        ...
        print("[TS_COM]: Connection request from client {}".format(address))
        try: 
            clientdata = client.recv(maxSize).decode('utf-8')
            while clientdata or 0:
                data = checkTable(clientdata, dns_table)
                print("[TS_COM]: Client sent {} | Server sending {} to client {}".format(clientdata,data, address))
    ...
    print("[TS_COM]: Socket Connection Closed")	
    ss.close()
    exit()
...
if __name__ == "__main__":
    if (len(sys.argv) > 2):
        ...
        print("python ts_com.py tsListenPort")
        exit()
    if (len(sys.argv) < 2):
