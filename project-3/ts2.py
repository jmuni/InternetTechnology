...
# ts2ListenPort_a ---> Used to connect with AS
# ts2ListenPort_c ---> Used to connect with CLIENT
...
def connectAS():
    ...
    msg = "AS to TS2 Connection Successful!"
...
def connectClient():
    ...
    #send a message to client
    msg = "Client to TS2 Connection Successful!"
    client.send(msg.encode('utf-8'))
    return client

def openFile():
    print("---------File Things-------")
    f = open("PROJ3-DNSTS2.txt", "r")
    print (f.name)
    f1 = f.readlines()
    for x in f1:
        print(x)
